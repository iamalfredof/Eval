...
private
  ...
  def process!(non_optimized = false)
    ...
    if office_flag
      ...
      %x( cp #{file_path_path} #{folder}/#{simple_file_name} )
      unless $?.exitstatus == 0
        Rails.logger.error "Failed at copying converted office2pdf to folder. Command: cp #{file_path_path} #{folder}/#{simple_file_name}"
        return false
      end
    end
